...
class SafetyPolicy:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "name": self.name,
            "allowedActions": sorted(self.allowed_actions) if self.allowed_actions is not None else None,
...
def load_policy_file(path: Path) -> SafetyPolicy:
    ...
    base.require_dry_run_actions = _string_set(raw.get("requireDryRunActions"), base.require_dry_run_actions) or set()
...
def _check_window(policy: SafetyPolicy, action: dict[str, Any]) -> None:
    ...
    if denied:
        raise PermissionError(f"blocked by policy: window title matches denied substring {denied!r}")

    if policy.allowed_window_title_substrings:
        allowed = _matches_any(title, policy.allowed_window_title_substrings)
        if not allowed:
            raise PermissionError("blocked by policy: window title is not in allowedWindowTitleSubstrings")


def validate_action(policy: SafetyPolicy, action: dict[str, Any], *, dry_run: bool = False) -> dict[str, Any]:
    kind = _action_kind(action)
    if kind not in ALL_POLICY_ACTIONS:
        return {"policy": policy.to_dict(), "allowed": True, "reason": "unknown action validation deferred to dispatcher"}
    ...
    if kind in policy.require_dry_run_actions and not dry_run and not bool(action.get("dryRun", False)):
    ...
    if kind in {"click", "doubleClick", "rightClick", "move", "drag", "scroll"}:
        ...
        if kind == "scroll" and abs(int(action.get("clicks", 0))) > policy.max_scroll_abs:
